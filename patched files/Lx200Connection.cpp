...
void Lx200Connection::sendGoto(unsigned int ra_int, int dec_int)
{
	if (goto_commands_queued <= 1)
	{
		...
		if (ra >= 86400)
			ra -= 86400;
      // sendCommand(new Lx200CommandStopSlew(server)); // FS2 stores the stop an executes it right after the next GoTo and therefore kills this operation
...
void Lx200Connection::sendSync(unsigned int ra_int, int dec_int)
{
	if (goto_commands_queued <= 1)
	{
		...
		if (ra >= 86400)
			ra -= 86400;
      // sendCommand(new Lx200CommandStopSlew(server)); // FS2 might be problematic with this. So it's commented out. Has to be verified #TODO
		sendCommand(new Lx200CommandSetSelectedRa(server, ra));
		sendCommand(new Lx200CommandSetSelectedDec(server, dec));
